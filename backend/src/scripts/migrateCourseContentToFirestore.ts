...
import { coursesCollection, modulesCollection, lessonsCollection, quizzesCollection, assignmentsCollection } from '../firebase/collections';
...
const loadJson = (filename: string): any => {
  const primaryPath = path.join(dataDir, filename);
  ...
  const backupPath = path.join(dataDir, 'syllabus_backup', filename);
...
async function saveModuleAndLesson(
  courseId: string,
  modData: { id: string; title: string; order: number; duration: string; description?: string },
  lessonsData: Array<{ id: string; title: string; order: number; duration: string; type: string; content: string; description?: string; readingTime?: string }>
) {
  // 1. Top-level modules collection
  await modulesCollection().doc(modData.id).set(
    toDocument({
      ...modData,
      courseId,
      updatedAt: new Date(),
  ...
  for (const les of lessonsData) {
    ...
    // Top-level lessons collection
    await lessonsCollection().doc(les.id).set(toDocument(lessonDoc), { merge: true });
    ...
    await db
      ...
      .collection('lessons')
      .doc(les.id)
      .set(toDocument(lessonDoc), { merge: true });
...
/**
 * 1. Migrate C Programming Course
 */
async function migrateCCourse() {
  const courseId = 'c-programming-course-id';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const cSyllabusNotes = loadJson('c_syllabus_data.json');
    const modulesData = [
    ...
    for (const mod of modulesData) {
      ...
      await saveModuleAndLesson(courseId, mod, [
        {
          id: lessonId,
          title: lessonTitle,
    ...
    await coursesCollection().doc(courseId).set(
      toDocument({
        id: courseId,
        title: 'C Programming',
        slug: 'c-programming',
    ...
    console.log(`[MIGRATION] Successfully migrated C Programming.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`C Programming: ${err.message}`);
    console.error(`[MIGRATION ERROR] C Programming:`, err);
  }
}

/**
 * 2. Migrate Git & GitHub Mastery Course
 */
async function migrateGitCourse() {
  const courseId = 'git-github-mastery-course-id';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const gitSyllabusNotes = loadJson('git_syllabus_data.json');
    const modulesData = [
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated Git & GitHub Mastery.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`Git Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] Git Course:`, err);
  }
}

/**
 * 3. Migrate Python Through OOPs Course
 */
async function migratePythonCourse() {
  const courseId = 'python-through-oops-course-id';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const pythonSyllabusNotes = loadJson('python_syllabus_data.json');
    const modulesData = [
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated Python Through OOPs.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`Python Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] Python Course:`, err);
  }
}

/**
 * 4. Migrate Java Through OOPs Course
 */
async function migrateJavaCourse() {
  const courseId = 'java-through-oops-course-id';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const javaSyllabusNotes = loadJson('java_syllabus_data.json');
    const modulesData = [
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated Java Through OOPs.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`Java Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] Java Course:`, err);
  }
}

/**
 * 5. Migrate Database Management System (DBMS) Course
 */
async function migrateDbmsCourse() {
  const courseId = 'database-management-system';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const modulesData = [
    ...
    for (const mod of modulesData) {
      const lessons = lessonsData[mod.id] || [];
      await saveModuleAndLesson(courseId, mod, lessons);
    }
    ...
    for (const quiz of quizzesData) {
      await quizzesCollection().doc(quiz.id).set(
        toDocument({
          ...quiz,
          courseId,
          updatedAt: new Date(),
    ...
    for (const assign of assignmentsData) {
      await assignmentsCollection().doc(assign.id).set(
        toDocument({
          ...assign,
          courseId,
          updatedAt: new Date(),
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated DBMS.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`DBMS Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] DBMS Course:`, err);
  }
}

/**
 * 6. Migrate Kubernetes Complete Course
 */
async function migrateKubernetesCourse() {
  ...
  try {
    ...
    const lessonsList = [
      ...
      },
      {
        ...
        lessons: [
          ...
          { id: 'k8s-unit-4-3', title: '4.3 Environment Variables', order: 4, duration: '40 mins', type: 'reading', content: '## Env Variables\nInject configurations into container environments from ConfigMaps/Secrets.' },
    ...
    for (const group of lessonsList) {
      const mod = modulesData.find((m) => m.id === group.moduleId);
      if (mod) {
        await saveModuleAndLesson(courseId, mod, group.lessons);
      }
    }
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated Kubernetes.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`Kubernetes Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] Kubernetes Course:`, err);
  }
}

/**
 * 7. Migrate React JS Complete Course
 */
async function migrateReactCourse() {
  const courseId = 'react-js-complete-course';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const jsonContent = loadJson('react_js_complete_course_content.json');
    ...
    for (const m of jsonContent.modules) {
      const mod = modulesData.find((modItem: any) => modItem.order === m.moduleNumber);
      if (mod) {
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated React JS.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`React JS Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] React JS Course:`, err);
  }
}

/**
 * 8. Migrate Linux Systems & Administration Mastery Course
 */
async function migrateLinuxCourse() {
  const courseId = 'course_linux_101';
  console.log(`[MIGRATION] Migrating ${courseId}...`);
  try {
    const jsonContent = loadJson('Linux_Complete_Course_Content.json');
    ...
    for (const m of jsonContent.modules) {
      const modTitle = userRequestedTitles[m.moduleNumber - 1] || m.title;
      const mod = {
      ...
      const lessonTitle = `${modTitle} - Complete Notes`;
      const lessonDesc = `${modTitle} Complete Notes.`;
      ...
      modulesForCourseDoc.push({
        id: `linux-mod-${m.moduleNumber}`,
        title: modTitle,
        description: modTitle,
        duration: '4 Hours',
        topics: [
          {
            id: `linux-topic-${m.moduleNumber}`,
            title: `${modTitle} - Complete Notes`,
            description: `${modTitle} Complete Notes.`,
    ...
    stats.coursesMigrated++;
    console.log(`[MIGRATION] Successfully migrated Linux Systems & Administration Mastery.`);
  } catch (err: any) {
    stats.failedRecords++;
    stats.errors.push(`Linux Course: ${err.message}`);
    console.error(`[MIGRATION ERROR] Linux Course:`, err);
  }
}
...
export async function runMigration(): Promise<MigrationStats> {
  console.log('====================================================');
  console.log('🚀 Starting KaizenQ LMS Course Content Migration to Firestore...');
  console.log('====================================================');

  if (!isFirebaseAdminInitialized()) {
    console.warn('⚠️ Firebase Admin SDK is not initialized. Check your credentials.');
  }

  await migrateCCourse();
  await migrateGitCourse();
  await migratePythonCourse();
  await migrateJavaCourse();
  await migrateDbmsCourse();
  await migrateKubernetesCourse();
  await migrateReactCourse();
  await migrateLinuxCourse();

  console.log('====================================================');
  console.log('✅ Migration Process Completed Summary:');
  console.log('----------------------------------------------------');
  ...
  console.log(`Quizzes Migrated     : ${stats.quizzesMigrated}`);
  ...
  if (stats.failedRecords > 0) {
    console.error('Errors encountered during migration:');
