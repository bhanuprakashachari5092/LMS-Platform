...
// Helper function mock to mimic LmsCourseRenderer parsing
function parseBlocks(content: string, courseId: string) {
  ...
  return { cleanContent, blocksCount: groupedBlocks.length, blocks: groupedBlocks.slice(0, 10) };
}
...
async function main() {
  ...
  const result = parseBlocks(content, courseId);
  console.log(`Cleaned content length: ${result.cleanContent.length}`);
  console.log(`Blocks parsed: ${result.blocksCount}`);
